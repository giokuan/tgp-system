<div>
    ...
    <div wire:key = "{{ $members->id }}" class="container p-5 mx-auto my-5 dark:bg-dark-eval-1">
        <div class="md:flex no-wrap md:-mx-2 ">
            <!-- Left Side -->

            <input wire:model="user_id" type="hidden" value="{{ $members->user_id }}" name="user_id" />
            ...
            <div class="w-full md:w-3/12 md:mx-2 ">
                ...
                <div class="p-3 bg-white dark:bg-dark-eval-1 hover:shadow">
                    ...
                    <div class="grid dark:bg-dark-eval-1">
                        <div class="my-2 text-center h-72 w-72">
                            <img class="w-full h-full" src="{{ asset('storage/' . $members->photo) }}" alt="">
                ...
                <div class="p-3 bg-white border-t-4 border-gray-500 dark:bg-dark-eval-1">
                    ...
                    <ul
                        class="px-3 py-2 mt-3 text-gray-600 bg-gray-100 divide-y rounded shadow-xl dark:bg-dark-eval-1 hover:text-gray-700 hover:shadow">
                        <li class="flex items-center py-3 font-bold dark:text-teal-600 md:text-xs">
                            <span>STATUS</span>
                            @if ($members->status === 'ACTIVE')
                                <span class="ml-auto"><span
                                        class="px-2 py-1 text-white bg-green-500 rounded md:text-xs">{{ $members->status }}</span></span>
                            @elseif($members->status === 'INACTIVE')
                                <span class="ml-auto"><span
                                        class="px-2 py-1 text-white bg-blue-500 rounded md:text-xs">{{ $members->status }}</span></span>
                            @else
                                <span class="ml-auto"><span
                                        class="px-2 py-1 text-white bg-red-500 rounded md:text-xs">{{ $members->status }}</span></span>
                            @endif
                        </li>
                        <li class="flex flex-col items-center py-3 dark:text-teal-600">
                            <span>MEMBER-ID:</span>
                            <span class="flex items-center md:text-xs">{{ $members->member_id }}</span>
                        </li>
                    </ul>
                </div>
                <!-- End of profile card -->
                    ...
                    <div class="text-gray-700">
                        <div class="grid text-sm md:grid-cols-1">
                            <div class="flex items-center shadow ">
                                ...
                                <div class="py-2 pl-4 dark:text-gray-500">{{ $members->first_name }}
                                    {{ $members->middle_name }} {{ $members->last_name }}</div>
                            </div>
                            <div class="flex items-center shadow ">
                                <div class="pl-2 font-semibold dark:text-teal-600">T-BIRTH:</div>
                                <div class="p-2 pl-4 dark:text-gray-500">{{ $members->t_birth }}</div>
                            </div>

                            <div class="flex items-center shadow ">
                                <div class="pl-2 font-semibold dark:text-teal-600">BATCH NAME:</div>
                                <div class="p-2 pl-4 dark:text-gray-500">{{ $members->batch_name }}</div>
                            </div>

                            <div class="flex items-center shadow ">
                                <div class="pl-2 font-semibold dark:text-teal-600">CURRENT CHAPTER:</div>
                                <div class="p-2 pl-4 dark:text-gray-500">{{ $members->current_chapter }}</div>
                            </div>

                            <div class="flex items-center shadow ">
                                <div class="pl-2 font-semibold dark:text-teal-600">ROOT CHAPTER:</div>
                                <div class="p-2 pl-4 dark:text-gray-500">{{ $members->root_chapter }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div bg-white dark:bg-dark-eval>
                        ...
                        <ul class="space-y-2 ">
                            <li>
                                ...
                                <div class="text-sm text-gray-500">{{ $members->email }}</div>
                            </li>
                            <li>
                                <div class="text-teal-600">PHONE</div>
                                <div class="text-sm text-gray-500">{{ $members->phone }}</div>
                            </li>

                            <li>
                                <div class="text-teal-600">ADDRESS</div>
                                <div class="text-sm text-gray-500">{{ $members->address }}</div>
                            </li>


                        </ul>
                    </div>
                    <div class="pt-8 pb-8 dark:bg-dark-eval">
